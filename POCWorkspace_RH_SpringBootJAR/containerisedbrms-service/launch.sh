...
if [ "$MVN_ARG_LINE" != "" ] ; then
    ...
    "$mvnBin" -v
    echo
    projects=( "*-model" "*-kjar" "containerisedbrms-service")
...
if [[ "$@" =~ "docker" ]]; then
    ...
    docker run -d -p 8090:8090 --name containerisedbrms-service apps/containerisedbrms-service:1.0-SNAPSHOT
elif [[ "$@" =~ "openshift" ]]; then
    echo "Launching the application on OpenShift..."
    
    oc new-app containerisedbrms-service:1.0-SNAPSHOT
    oc expose svc/containerisedbrms-service
else

	echo "Launching the application locally..."
	pattern="containerisedbrms-service"
	files=( $pattern )
	cd ${files[0]}
